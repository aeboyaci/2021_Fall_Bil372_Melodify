...
const Login = () => {
    ...
    const handleSubmit = (values, {resetForm}) => {
        axios.post("http://localhost:3001/api/account/login", {...values}, {withCredentials: true}).then((d) => {
            console.log(d.data);
            setAuth({Username: d.data.Username, Role: d.data.Role});
            history.push("/home");
            resetForm();
        }).catch(() => {
            setMessage("E-mail address or password is wrong.");
        });
    };
        ...
        <Grid container sx={{height: "100vh", width: "100%"}} justifyContent={"center"} alignItems={"center"}>
            <Grid item display={"flex"} flexDirection={"column"} xs={12} sm={6} justifyContent={"center"} alignItems={"center"}>
                <Grid item xs={12} sm={6} sx={{pb: 8, px: 2}}>
                    <Typography textAlign={"center"} sx={{mb: 4}} variant={"h3"}>Login</Typography>
                    ...
                    <CustomForm onSubmit={handleSubmit} validationSchema={validationSchema} fields={fields} buttonText={"Login"} initialValues={{Email: "", Password: ""}} />
                    <Box sx={{mt: 2}}>
                        <Typography textAlign={"center"}>New to Melodify? <Link  to={"/account/register"} style={{textDecoration: "none", color: "#1DB954"}}>Join now!</Link></Typography>
                    </Box>
                </Grid>
            </Grid>
